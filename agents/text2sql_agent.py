...
class Text2SQLAgent:
    ...
    def generate_sql(self, state: GraphState) -> GraphState:
        ...
        prompt = (
            ...
            "1) `zoho_deals` is the primary root table for Trimstone projects. The `created_by` column shows the owner name.\n"
            "   - When you need user information for a deal/project owner, JOIN using `zoho_deals.owner_email = [user].email` (use the `user` table).\n"
            "   - To connect Zoho deals to other systems, JOIN `zoho_deals.id = project.hubspot_id` (use the `project` table as the bridge).\n"
            "2) `xero_projects` indicates billing records for projects. To find billing details for a Zoho deal project, JOIN via the `project` table:\n"
            "   - `zoho_deals.id = project.hubspot_id` AND `xero_projects.project_id = project.xero_id`.\n"
            "3) `zoho_deals` is connected to `zoho_accounts` via `zoho_deals.zoho_account_id = zoho_accounts.id`. Use this join when account-level information is requested.\n"
            "4) `zoho_contacts` maps to `contacts` via `zoho_contacts.id = contacts.hubspot_id`. Prefer this join when resolving contact details.\n"
