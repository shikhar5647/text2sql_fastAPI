...
class ValidatorAgent:
    ...
    def validate_sql(self, state: GraphState) -> GraphState:
        ...
        try:
            # Guard: ensure we have a non-empty string SQL before running checks
            if not sql_query or not isinstance(sql_query, str):
                state["is_valid"] = False
                state["validation_message"] = "No SQL generated by LLM (empty or null)."
                state["safety_check"] = False
                state["step"] = "validation_failed"
                logger.warning("No SQL generated by LLM; skipping validation")
                return state
